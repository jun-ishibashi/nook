...
export default function ShareButtons({ title, url }: { title: string; url: string }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <a href={`https://twitter.com/intent/tweet?text=${encodedTitle}&url=${encodedUrl}`} target="_blank" rel="noopener noreferrer" className={btnClass} style={{ color: "var(--text-muted)" }} aria-label="Xでシェア">
      ...
      <a href={`https://social-plugins.line.me/lineit/share?url=${encodedUrl}`} target="_blank" rel="noopener noreferrer" className={btnClass} style={{ color: "var(--text-muted)" }} aria-label="LINEでシェア">
      ...
      <button type="button" onClick={copyLink} className={btnClass} style={{ color: copied ? "#6b8a68" : "var(--text-muted)" }} aria-label="リンクをコピー">
        {copied ? (
