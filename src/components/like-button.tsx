...
export default function LikeButton({ postId, initialLiked, initialCount, size = "md" }: {
  postId: string; initialLiked: boolean; initialCount: number; size?: "sm" | "md";
}) {
  ...
  return (
    <button
      type="button" onClick={toggleLike} disabled={isPending}
      className={`group inline-flex items-center gap-1 transition active:scale-90 ${size === "sm" ? "px-1 py-1 text-[11px]" : "px-2 py-1.5 text-xs"}`}
      style={{ color: liked ? "#c45c5c" : "var(--text-muted)" }}
