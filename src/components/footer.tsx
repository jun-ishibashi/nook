...
const footerNavLinkClass =
  "inline-flex min-h-11 items-center rounded-sm px-2 transition hover:opacity-75 focus-visible:outline-offset-2 sm:min-h-9 sm:px-1";

export default function Footer() {
  return (
    <footer
      className="mt-auto nook-app-canvas border-t nook-border-hairline"
      aria-label="サイトフッター"
    >
      <div className="nook-page flex flex-col items-center gap-2.5 pt-7 pb-[max(1.75rem,env(safe-area-inset-bottom,0px))] sm:pt-8 sm:pb-[max(2rem,env(safe-area-inset-bottom,0px))]">
        <Link
          href="/"
          className="nook-fg-muted flex min-h-[var(--touch)] items-center gap-1.5 rounded-sm text-sm font-semibold tracking-[0.06em] focus-visible:outline-offset-2 sm:min-h-0 sm:text-xs"
        >
          <Logo size={16} />
          NOOK
        </Link>
