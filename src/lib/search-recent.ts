...
const RECENT_UPDATED = "nook-recent-searches";
...
export function subscribeRecentSearches(onStoreChange: () => void): () => void {
  ...
  window.addEventListener(RECENT_UPDATED, onCustom);
  return () => {
    window.removeEventListener("storage", onStorage);
    window.removeEventListener(RECENT_UPDATED, onCustom);
  };
}
...
export function saveRecentSearch(query: string): void {
  ...
  try {
    ...
    window.dispatchEvent(new Event(RECENT_UPDATED));
  } catch {
    /* ignore */
  }
}
